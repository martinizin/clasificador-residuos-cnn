...
10. [Problemas Conocidos y Mejoras](#problemas-conocidos-y-mejoras)
11. [Guía para Defensa del Proyecto](#guía-para-defensa-del-proyecto)
12. [Licencia y Créditos](#licencia-y-créditos)
13. [TL;DR — Comandos Rápidos](#tldr--comandos-rápidos)
...
1. **Desbalance de clases**: `trash` tiene ~137 imágenes vs `paper` con ~594.
   - Impacto: El modelo puede sesgarse hacia clases mayoritarias.
   
2. **Domain shift**: Las imágenes del dataset son de estudio (fondo blanco, buena luz).
   - Impacto: En fotos reales (basurero, poca luz), el rendimiento baja.

3. **Confusión entre clases similares**:
   - Botellas de vidrio vs. plástico.
   - Cartón vs. papel.

4. **Umbral de confianza fijo**: Siempre predice algo, aunque la confianza sea 30%.

### Mejoras sugeridas

| Mejora                     | Implementación                                      | Dificultad |
|----------------------------|-----------------------------------------------------|------------|
| Class weights              | `model.fit(..., class_weight={0:1.2, 5:2.0})`       | Fácil      |
| Más augmentations          | `RandomBrightness`, `RandomContrast`, `CutMix`      | Media      |
| Threshold de confianza     | Si `max(preds) < 0.5`, mostrar "No seguro"          | Fácil      |
| Confusion matrix           | `sklearn.metrics.confusion_matrix` en evaluación    | Fácil      |
| Fine-tuning parcial        | Descongelar solo últimas 50 capas                   | Media      |
| Más datos                  | Augmentation offline, recolectar imágenes propias   | Alta       |
| Calibración                | Temperature scaling post-entrenamiento              | Media      |
| Ensemble                   | Combinar MobileNetV2 + EfficientNet                 | Alta       |

### Código ejemplo: Class weights
...
# Ejemplo: {0: 1.2, 1: 0.9, 2: 1.1, 3: 0.7, 4: 0.9, 5: 2.5}

model.fit(train_ds, validation_data=val_ds, epochs=epochs, class_weight=class_weights)
```

### Código ejemplo: Threshold de confianza

```python
# En app.py, después de obtener preds
confidence = float(preds[top])
if confidence < 0.5:
    st.warning(f"⚠️ Confianza baja ({confidence:.2%}). La predicción podría no ser confiable.")
```
...
|-----------|-----------------------------------|------------------------------------------------|
| 0:00-0:30 | Introducción                      | Problema, motivación, objetivo                 |
| 0:30-1:30 | Dataset y preprocesamiento        | Fuente, clases, split, augmentation            |
| 1:30-3:00 | Arquitectura del modelo           | CNN, Transfer Learning, MobileNetV2            |
| 3:00-4:00 | Entrenamiento                     | Fases (congelado + fine-tuning), métricas      |
| 4:00-5:00 | Demo en vivo                      | Subir imagen, mostrar predicción               |
| 5:00-6:00 | Limitaciones y mejoras            | Desbalance, domain shift, próximos pasos       |
| 6:00-7:00 | Conclusiones                      | Resumen, aprendizajes                          |

---

### Preguntas frecuentes del jurado

#### ¿Por qué usaste CNN y no otra arquitectura?

> "Las CNNs son el estándar para visión por computador porque:
> 1. Extraen características jerárquicas (bordes → texturas → objetos).
> 2. Son invariantes a la posición del objeto en la imagen.
> 3. Tienen menos parámetros que redes densas gracias a la convolución.
> 
> Para clasificación de imágenes con ~2,500 muestras, CNN + Transfer Learning es la combinación óptima."

#### ¿Qué es Transfer Learning y por qué MobileNetV2?

> "Transfer Learning es reutilizar conocimiento aprendido en un problema (ImageNet) para resolver otro (clasificación de residuos). MobileNetV2 fue elegida porque:
> - Es ligera (~14MB vs ~500MB de VGG16).
> - Es rápida en CPU (ideal para deploy en Streamlit Cloud).
> - Tiene buena precisión (71.8% en ImageNet).
> - Sus 'depthwise separable convolutions' reducen cómputo sin perder calidad."

#### ¿Cómo preparaste los datos?

> "Los datos provienen del dataset 'Garbage Classification' de Kaggle. El preprocesamiento incluye:
> 1. Organización en 6 carpetas (una por clase).
> 2. Split 80/20 para entrenamiento/validación (con semilla fija).
> 3. Redimensionamiento a 224×224 píxeles.
> 4. Data augmentation: flip horizontal, rotación ±18°, zoom ±10%.
> 5. Normalización a rango [-1, 1] con `preprocess_input`."

#### ¿Qué significa accuracy y qué limitaciones tiene?

> "Accuracy es el porcentaje de predicciones correctas. Limitación: si el dataset está desbalanceado (trash tiene pocas imágenes), un modelo que ignore esa clase aún tendría alto accuracy. Alternativas: precision, recall, F1-score, confusion matrix."

#### ¿Qué limitaciones tiene el dataset?

> "1. **Desbalance**: trash tiene ~137 imágenes vs paper con ~594.
> 2. **Domain shift**: Las fotos son de estudio; en contextos reales (basura real, mala luz), el rendimiento baja.
> 3. **Ambigüedad**: Botellas de vidrio vs plástico son visualmente similares.
> 4. **Tamaño**: ~2,500 imágenes es pequeño para deep learning."

#### ¿Qué harías para mejorar la clasificación de botellas?

> "1. Agregar más imágenes de botellas con etiquetas correctas.
> 2. Usar augmentation específica (cambios de brillo, reflejos).
> 3. Fine-tuning más agresivo en las capas superiores.
> 4. Class weights para equilibrar plástico vs vidrio.
> 5. Capturar imágenes propias en contextos reales."

#### ¿Cómo funciona el deploy en Streamlit?

> "Streamlit Cloud:
> 1. Conectas tu repo de GitHub.
> 2. Seleccionas el archivo `app.py`.
> 3. Streamlit instala dependencias de `requirements.txt`.
> 4. Ejecuta `streamlit run app.py` en la nube.
> 5. Proporciona una URL pública para acceso."

#### ¿Qué pasa si cambian las clases o el tamaño de imagen?

> "El sistema está diseñado para ser flexible:
> - `labels.json` guarda el orden de clases; si se re-entrena, se actualiza.
> - `meta.json` guarda `img_size`; la app lo lee dinámicamente.
> - Si agrego una clase 'organic', solo debo agregar la carpeta y re-entrenar; los JSONs se actualizan automáticamente."

#### ¿Cómo aseguras reproducibilidad?

> "1. **Semilla fija** (`seed=1337`) para el split de datos.
> 2. **requirements.txt** con versiones fijas (se puede añadir `tensorflow==2.16.1`).
> 3. **Guardado de metadatos** (labels.json, meta.json).
...
| Tipo de imagen                  | Propósito                                      |
|---------------------------------|------------------------------------------------|
| Del dataset (no usada en train) | Demostrar que funciona en datos conocidos      |
| Foto propia de objeto real      | Demostrar generalización                       |
| Caso difícil (botella de vidrio)| Mostrar limitaciones y discutir mejoras        |
| Imagen irrelevante (un gato)    | Mostrar que el modelo da baja confianza        |

#### Buenas prácticas

- ✅ Tener imágenes descargadas previamente (no depender de internet).
- ✅ Usar imágenes con buena iluminación y fondo limpio.
- ✅ Preparar una imagen que falle para discutir limitaciones.
- ✅ Mostrar el top-3 para evidenciar la incertidumbre del modelo.
- ✅ Tener backup offline (video grabado) por si falla internet.

#### Errores a evitar

- ❌ Usar una imagen del set de entrenamiento (es trampa).
- ❌ Probar solo imágenes "fáciles".
- ❌ No saber explicar por qué falló una predicción.
- ❌ Depender de conexión a internet sin backup.
...
- **Nombre**: Garbage Classification (6 classes)
- **Fuente**: [Kaggle](https://www.kaggle.com/datasets/asdasdasasdas/garbage-classification)
- **Licencia**: Verificar en Kaggle (uso educativo).

### Librerías utilizadas

| Librería    | Uso                          | Licencia   |
|-------------|------------------------------|------------|
| TensorFlow  | Framework de deep learning   | Apache 2.0 |
| Keras       | API de alto nivel para redes | Apache 2.0 |
| Streamlit   | Interfaz web                 | Apache 2.0 |
| Pillow      | Manipulación de imágenes     | HPND       |
| NumPy       | Operaciones numéricas        | BSD        |

### Autor

Proyecto desarrollado para la materia **Inteligencia Artificial I** — Semestre 2026-10.
...
# 3. Entrenar el modelo (asegúrate de tener data/raw/ con las 6 carpetas)
python train.py --epochs 5 --fine_tune_epochs 3
...
**¿Dudas?** Abre un issue en el repositorio o contacta al autor.

♻️ *Clasificando residuos, un modelo a la vez.* ♻️
